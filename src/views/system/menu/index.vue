    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" destroy-on-close append-to-body width="1000px"
      ...
      <el-form ref="menuFormRef" :model="formData" :rules="rules" label-width="160px">
        ...
        <el-form-item label="菜单名称" prop="name">
          <el-input v-model="formData.name" placeholder="請輸入菜单名称" />
        </el-form-item>
...
const rules = reactive({
  ...
  name: [{ required: true, message: "請輸入菜单名称", trigger: "blur" }],
  type: [{ required: true, message: "请选择菜单类型", trigger: "blur" }],
  path: [{ required: true, message: "請輸入路由路径", trigger: "blur" }],

  component: [{ required: true, message: "請輸入组件路径", trigger: "blur" }],
  visible: [{ required: true, message: "請輸入路由路径", trigger: "blur" }],
});
