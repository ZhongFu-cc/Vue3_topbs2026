...
/**
 * export { }被用來確保 TypeScript 能夠將這個檔案視為一個模組。即使這個檔案中只有聲明，沒有實際的程式碼，
 * TypeScript 仍然希望每個檔案都作為一個獨立的模組。
 * 當一個檔案包含至少一個 import 或 export 聲明時，TypeScript 會將它視為一個模組，並進行模組化處理。
 * 因此，即使這個檔案只是用來定義類型或擴展其他模組，也需要確保它被視為一個模組。
 * 
 */
export { }

//擴展vue模組,讓被登入的全局組件確認類型,就是方便在編寫時能有個提醒,提醒這是Vue組件
//這個在tsconfig.json中有被incloud 用於幫助TS判斷,
//雖然全局組件註冊是寫在vite.config中的 Components => dirs 配置項, 但是在編譯時要有提醒,需要在這邊填寫組件名以及引用類型
//並在tsconfig.json incloud才行
declare module "vue" {
  export interface GlobalComponents {

    //組件名: 引用類型 ['default']來獲取模組的默認導出,以下是框架預設的組件類型,不要動到, 要添加請寫最上面或最下面
